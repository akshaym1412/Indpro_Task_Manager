...
const EditTaskForm = ({ setEditTask, task,id,fetchTasks }) => {
    console.log(task)
...
<div className="flex flex-col lg:grid grid-cols-3 gap-4">
  ...
  <div>
    ...
    <div className="flex gap-2">
      <button
        type="button"
        className={`border border-gray-300 px-6 py-2 rounded-3xl ${tasks.category === "Work" ? "bg-purple-500 text-white" : ""}`}
      ...
      <button
        type="button"
        className={`border border-gray-300 px-6 py-2 rounded-3xl ${tasks.category === "Personal" ? "bg-purple-500 text-white" : ""}`}
        onClick={() => setTasks({ ...task, category: "Personal" })}
      >
        Personal
      </button>
    </div>
...
<div className="-m-5 pr-5 lg:pr-10 flex justify-end items-center mt-6 gap-4 bg-[#f1f1f1] h-20 rounded-b-4xl">
  <button type="reset" className="px-4 py-2 border border-gray-400 rounded-3xl uppercase cursor-pointer h-fit"  onClick={() => setEditTask(false)}>
    Cancel
  </button>
